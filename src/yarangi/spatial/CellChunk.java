...
public class CellChunk implements IAreaChunk
{
	...
	public CellChunk(Area area, double x, double y, double cellsize)
	{
		...
		this.minx = Math.max(x - halfsize, area.getAnchor().x()-area.getMaxRadius());
		this.maxx = Math.min(x + halfsize, area.getAnchor().x()+area.getMaxRadius());
		this.miny = Math.max(y - halfsize, area.getAnchor().y()-area.getMaxRadius());
		this.maxy = Math.min(y + halfsize, area.getAnchor().y()+area.getMaxRadius());
	}
